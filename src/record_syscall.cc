 ...
 * pointer passed in. Return TRUE if there's no scrach overflow.
 */
static bool reserve_scratch_for_msghdr(Task *t, struct msghdr *msg, byte **scratch)
{
	...
	*scratch = scratch_tmp;
	...
	if (msg->msg_control) {
		t->remote_memcpy(msg->msg_control, msg->msg_control,
				 msg->msg_controllen);
	}
	return true;
}
...
"  this process exits.\n"
	...
	case SYS_recvmmsg: {
		...
		for (int i = 0; i < vlen; ++i, pmsgvec++)
		{
			t->read_mem(pmsgvec, &msgvec[i]);
		}
		...
		for (int i = 0; i < vlen; ++i, tmpmsgvec++)
		{
			t->write_mem(tmpmsgvec, msgvec[i]);
		}
		t->set_regs(r);
...
static void record_and_restore_msghdr(Task* t, struct msghdr *dst, struct msghdr *src)
{
	...
	t->write_mem(dst, msg);
	t->record_local(dst, sizeof(tmpmsg), &msg);
