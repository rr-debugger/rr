...
static void child(void) {
	...
	for (i = 1; i < NUM_MSGS; ++i) {
		...
		atomic_printf("%s: ... returned %d (%s/%d): (%ld, %ld)\n", id,
			      ret, strerror(err), err, msg.mtype, msg.msg);
...
int main(int argc, char *argv[]) {
	...
	atomic_printf("%s: Q stats in %p:\n"
		      ...
		      id, &buf, buf.msg_perm.mode, buf.msg_qnum,
	...
	atomic_printf("%s: IPC_INFO returned %d (%s/%d):\n"
		      "    { max:%d mnb:%d mni:%d }\n", id,
		      ret, strerror(err), err,
		      info.msgmax, info.msgmnb, info.msgmni);
	test_assert(ret >= 0);
	...
	atomic_printf("%s: MSG_INFO returned %d (%s/%d):\n"
		      "    { pool:%d map:%d tql:%d }\n", id,
		      ret, strerror(err), err,
		      info.msgpool, info.msgmap, info.msgtql);
	test_assert(ret >= 0);
	...
	atomic_printf("%s: Q stats: { perm:%#o qnum:%ld lspid:%d lrpid:%d }\n",
		      id, buf.msg_perm.mode, buf.msg_qnum,
		      buf.msg_lspid, buf.msg_lrpid);
