...
bool ReplayTimeline::InternalMark::equal_states(
    const InternalMark& mark) const {
  if (this == &mark) {
    return true;
  }
  if (ticks_at_event_start != mark.ticks_at_event_start) {
    return false;
  }

  return proto.equal_states(mark.proto);
}
...
bool ReplayTimeline::ProtoMark::equal_states(
    const ReplayTimeline::ProtoMark& proto) const {
  if (key != proto.key) {
    return false;
  }
  return equal_regs(regs, proto.regs) &&
         return_addresses == proto.return_addresses;
}
...
ReplayTimeline::Mark ReplayTimeline::recreate_mark_from_data(
    const MarkData& mark_data, ReplaySession::shr_ptr session) {
  Mark result;
  auto m = make_shared<InternalMark>(this, mark_data, session);
  m->inc_refcount();
  auto& mark_vector = marks[m->proto.key];
  // If we are recreating a mark, in the same debug sesssion where we actually
  // created the persistent checkpoint in there's a pretty high likelihood that
  // the same `InternalMark` still exists for that original checkpoint

  for (std::shared_ptr<InternalMark>& internal_mark : mark_vector) {
    if (internal_mark->equal_states(*m)) {
      // swap out the existing mark with this one - they're the same, but `m`
      // contains the deserialized session
      internal_mark = m;
      result.ptr = internal_mark;
      return result;
    }
  }
...
void ReplayTimeline::register_mark_as_checkpoint(Mark& m) {
  DEBUG_ASSERT(m.ptr && m.ptr->checkpoint &&
               "Can't register mark as checkpoint if no checkpoint exists");
...
ReplayTimeline::Mark ReplayTimeline::recreate_marks_for_non_explicit(
    const CheckpointInfo& cp, std::shared_ptr<ReplaySession> clone) {
  DEBUG_ASSERT(cp.non_explicit_mark_data && clone.get() != nullptr);
  // first add the mark with an actual clone, this is not a GDB checkpoint, but
  // an RR checkpoint
  ...
  // then add the mark with no clone, the one that will be visible to GDB, i.e.
  // non explicit checkpoint
...
/* static */
ReplayTimeline::Progress ReplayTimeline::estimate_progress_of(
...
ReplayTimeline::ProtoMark ReplayTimeline::ProtoMark::from_serialized_markdata(
    const MarkData& md) {
  auto proto_mark = ProtoMark{ MarkKey{
      md.time, md.ticks, ReplayStepKey{ (ReplayTraceStepType)md.step_key } } };
  proto_mark.regs = md.regs;
  proto_mark.return_addresses = md.return_addresses;
  return proto_mark;
}

std::shared_ptr<ReplayTimeline::Mark>
ReplayTimeline::find_closest_mark_with_clone(const Mark& mark) {
  if (marks_with_checkpoints.empty())
    return nullptr;
  ...
  auto marks_found = std::find_if(std::cbegin(marks), std::cend(marks),
                                  [&](auto& kvp) { return kvp.first == *k; });

  if (marks_found == std::end(marks))
    return nullptr;

  for (auto it = std::rbegin(marks_found->second);
       it != std::rend(marks_found->second); it++) {
