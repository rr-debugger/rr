...
SignalHandled handle_signal(RecordTask* t, siginfo_t* si,
  ...
  if (deterministic == DETERMINISTIC_SIG) {
    t->handling_deterministic_signal = sig;
  }
  ...
  switch (sig) {
    case SIGSEGV:
      if (try_handle_rdtsc(t, si) || try_grow_map(t, si)) {
        // When SIGSEGV is blocked or ignored, apparently the kernel has to do
        // some ninjutsu to raise the trap.  We see the SIGSEGV bits in the
        // "SigBlk" and "SigIgn" masks in /proc/status cleared, and if
