 ...
 * Modified for use in libjulia (and then copied to rr):
...
#define CRC32C_USE_IFUNC

uint32_t crc32c_sw(uint32_t crci, const uint8_t *buf, size_t len);
typedef uint32_t (*crc32c_func_t)(uint32_t crc, const uint8_t *buf, size_t len);

uint64_t load_unaligned_i64(const void *ptr)
{
    uint64_t val;
...
static uint32_t crc32c_sse42(uint32_t crc, const uint8_t *buf, size_t len)
{
    ...
    /* compute the crc on the remaining eight-byte units less than a SHORT*3
       block */
    const uint8_t *end = buf + (len - (len & 7));
    while (buf < end) {
...
{
    // When used in ifunc, we cannot call external functions
...
CRC_TARGET static uint32_t crc32c_armv8(uint32_t crc, const char *buf, size_t len)
{
    ...
    while (len >= LONG * 3) {
        ...
        do {
            crc = crc32cx(crc, load_unaligned_i64(buf));
            buf += 8;
            crc1 = crc32cx(crc1, load_unaligned_i64(buf2));
            buf2 += 8;
            crc2 = crc32cx(crc2, load_unaligned_i64(buf3));
            buf3 += 8;
        } while (buf < end);
    ...
    if (len & 4) {
        crc = crc32cw(crc, load_unaligned_i32(buf));
        buf += 4;
    }
    if (len & 2) {
        crc = crc32ch(crc, load_unaligned_i16(buf));
        buf += 2;
    }
    if (len & 1)
        crc = crc32cb(crc, *buf);
...
#  ifdef CRC32C_USE_IFUNC
// ifunc dispatch
uint32_t crc32c(uint32_t crc, const uint8_t *buf, size_t len)
    __attribute__((ifunc (crc32c_dispatch_ifunc)));
#  else
// lazy wrapper dispatch
static uint32_t crc32c_lazy(uint32_t crc, const uint8_t *buf, size_t len);
static crc32c_func_t crc32c_func = crc32c_lazy;

static uint32_t crc32c_lazy(uint32_t crc, uint8_t *buf, size_t len)
{
...
/* Tables for hardware crc that shift a crc by LONG, SHORT and 4k zeros. */
static uint32_t crc32c_long[4][256];
static uint32_t crc32c_short[4][256];
static uint32_t crc32c_4k[4][256];
static uint32_t crc32c_1M[4][256];
static uint32_t crc32c_1G[4][256];
static uint32_t crc32c_1T[4][256];
...
static void crc32c_init_hw(void)
{
    ...
    crc32c_zeros(crc32c_4k, 4096);
    crc32c_zeros(crc32c_1M, 1024ULL * 1024ULL);
    crc32c_zeros(crc32c_1G, 1024ULL * 1024ULL * 1024ULL);
    crc32c_zeros(crc32c_1T, 1024ULL * 1024ULL * 1024ULL * 1024ULL);
}
...
int main(void)
{
    ...
    print_array("crc32c_4k", 4, 256, &crc32c_4k[0][0]);
    print_array("crc32c_1M", 4, 256, &crc32c_1M[0][0]);
    print_array("crc32c_1G", 4, 256, &crc32c_1G[0][0]);
    print_array("crc32c_1T", 4, 256, &crc32c_1T[0][0]);
    return 0;
}
