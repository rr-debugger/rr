...
enum ScratchAddrType {
  FIXED_ADDRESS, DYNAMIC_ADDRESS
};
/* Pointer used when running RR in WINE. Memory below this address is overwritten
by WINE so in that case the scratch buffer must begin here */
static const uintptr_t FIXED_SCRATCH_PTR = 0x68000000;

template <typename Arch> static void init_scratch_memory(Task* t, ScratchAddrType addr_type = DYNAMIC_ADDRESS) {
  ...
  int flags = MAP_PRIVATE | MAP_ANONYMOUS;
  {
    ...
    if (addr_type == DYNAMIC_ADDRESS ) {
      ...
      t->scratch_ptr =
        remote.mmap_syscall(remote_ptr<void>(FIXED_SCRATCH_PTR), sz, prot, flags | MAP_FIXED, -1, 0);
    }
