...
static void dump_statfs(const char* label, const struct statfs* s) {
	atomic_printf(
		...
		"}\n",
		label,
		(long)s->f_type, (long)s->f_bsize,
		s->f_blocks, s->f_bfree, s->f_bavail,
		s->f_files, s->f_ffree,
		s->f_fsid.__val[0], s->f_fsid.__val[1],
		(long)s->f_namelen, (long)s->f_frsize);
}
