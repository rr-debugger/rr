...
static FrameTime process_spawn_time(const string& trace_dir, pid_t pid) {
  ...
  FrameTime time = -1;
  for (auto e = trace.read_task_event(&time); e.type() != TraceTaskEvent::NONE;
       e = trace.read_task_event(&time)) {
    if ((e.type() == TraceTaskEvent::CLONE ||
         e.type() == TraceTaskEvent::EXEC) &&
        e.tid() == pid) {
...
static int replay(const string& trace_dir, const ReplayFlags& flags) {
  ...
  if (flags.dont_launch_debugger) {
    if (target.event == numeric_limits<decltype(target.event)>::max()) {
      ...
      if (!flags.ignore_persistent_cp) {
        ...
        if (cp != rend(cps)) {
          LOG(debug) << "Spawning from persistent checkpoint at "
                     << cp->event_time();
  ...
  if (0 == (waiting_for_child = fork())) {
...

    {
      ...
      auto session = ReplaySession::create(trace_dir, session_flags(flags, false));
      if (!flags.ignore_persistent_cp) {
        const auto event_when_created =
            process_spawn_time(trace_dir, flags.target_process);
        const auto pcps = session->get_persistent_checkpoints();
        const auto cp = find_if(rbegin(pcps), rend(pcps), [&](const auto& cp) {
          return target.event >= cp.event_time() ||
                 event_when_created > cp.event_time();
        });

        if (cp != rend(pcps)) {
          LOG(debug) << "Spawning session from persistent checkpoint at "
                     << cp->event_time();
          session->load_checkpoint(*cp);
        }
      }
